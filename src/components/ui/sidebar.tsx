...
>(
  (
    ...
    const { toggleSidebar: toggleSidebarShortcut } = useAppShortcuts()
    ...
    React.useEffect(() => {
      const handleKeyDown = (event: KeyboardEvent) => {
        if (matchesKeybind(event, toggleSidebarShortcut)) {
  ...
  (
    ...
    return (
      ...
      >
        ...
        <div
          className={cn(
            "fixed inset-y-0 z-30 hidden h-svh w-(--sidebar-width) bg-sidebar transition-[left,right,width] duration-200 ease-linear md:flex",
            side === "left"
